---
title: "`BulkSignalR` : </br> Inference of ligand-receptor interactions from bulk-rnaseq."
author:
...
abstract: > 
  <p> BulkSignalR is used to infer ligand-receptor (LR) interactions from bulk
   (transcriptomic or proteomic) data. BulkSignalR bases its inferences on the 
...
![id1](../man/figures/sdc_LR-null.png)
...
```{r scoringLR  ,cache=TRUE,results=FALSE}
...
![id3](../man/figures/sdc_scoresLR.svg)

###  Scoring by pathway

```{r scoringPathway, cache=TRUE,results=FALSE}    
...
![id4](../man/figures/sdc_scoresPathway.svg)
...
![id5](../man/figures/sdc_signatureheatmap.svg)
...
![id6](../man/figures/sdc_alluvial.svg)
...
![id7](../man/figures/sdc_bubble.svg)
...
```{r mouse , cache=TRUE,results=FALSE}
data(bodyMap.mouse)
...
# bsrsig.redBP <- getLRGeneSignatures(bsrinf.redBP, qval.thres=0.001)
# scoresLR <- scoreLRGeneSignatures(bsrdm,bsrsig.redBP,name.by.pathway=FALSE)
# simpleHeatmap(scoresLR[1:20,], file="sdc_scoreLR.pdf", column.names=TRUE, width=9, height=5, pointsize=8)
